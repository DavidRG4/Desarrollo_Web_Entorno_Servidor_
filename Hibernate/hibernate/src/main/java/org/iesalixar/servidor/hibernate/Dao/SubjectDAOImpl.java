...
public class SubjectDAOImpl extends GenericDAOImpl<Subject> implements SubjectDAOI {
	...
	public Subject searchByName(final String name) {
		...
		List<Subject> subjectList = session.createQuery("FROM Subject WHERE name=:name").setParameter("name", name).list();
